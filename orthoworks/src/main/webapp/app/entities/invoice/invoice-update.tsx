  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="orthoworksApp.invoice.home.createOrEditLabel" data-cy="InvoiceCreateUpdateHeading">
            <Translate contentKey="orthoworksApp.invoice.home.createOrEditLabel">Create or edit a Invoice</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('orthoworksApp.invoice.code')}
                id="invoice-code"
                name="code"
                data-cy="code"
                type="text"
              ...
              />
              <ValidatedField
                label={translate('orthoworksApp.invoice.date')}
                id="invoice-date"
                name="date"
                data-cy="date"
                type="datetime-local"
              ...
              />
              <ValidatedField
                label={translate('orthoworksApp.invoice.details')}
                id="invoice-details"
                name="details"
                data-cy="details"
                type="text"
              />
              <ValidatedField
                label={translate('orthoworksApp.invoice.status')}
                id="invoice-status"
                name="status"
                data-cy="status"
                type="select"
              >
                {invoiceStatusValues.map(invoiceStatus => (
                  <option value={invoiceStatus} key={invoiceStatus}>
                    {translate('orthoworksApp.InvoiceStatus.' + invoiceStatus)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('orthoworksApp.invoice.paymentMethod')}
              ...
              >
                {paymentMethodValues.map(paymentMethod => (
                  <option value={paymentMethod} key={paymentMethod}>
                    {translate('orthoworksApp.PaymentMethod.' + paymentMethod)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('orthoworksApp.invoice.paymentDate')}
              ...
              />
              <ValidatedField
                label={translate('orthoworksApp.invoice.paymentAmount')}
